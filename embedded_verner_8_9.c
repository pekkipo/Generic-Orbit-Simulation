...
//    
//     //Embedded_Verner_8_9( double (*f)(double, double), double y[], double x,
//                  //  double h, double xmax, double *h_next, double tolerance )
//     
//     
...
//     mrows = mxGetM(prhs[1]);
//     ncols = mxGetN(prhs[1]);
//     //_y = mxCreateDoubleMatrix(1,2,mxREAL);
//     _y = mxGetPr(plhs[0]);
//     _x = mxGetScalar(prhs[2]);
//     _h = mxGetScalar(prhs[3]);
//     _xmax = mxGetScalar(prhs[4]);
//     hnext = mxGetPr(prhs[5]);
//     _h = mxGetScalar(prhs[6]);
//    
//    result = Embedded_Verner_8_9(func, _y, _x, _h, _xmax, hnext, _tolerance);
//    
//    if(result != 0) {
...
//     }
  ...
  // OUTP = _y[1];//mxCreateDoubleMatrix(M, N, mxREAL);
  // B = mxGetPr(OUTP);
...
void mexFunction( int nlhs, mxArray *plhs[],
   ...
   func = mxGetPr(prhs[0]);
   epoch = mxGetScalar(prhs[2]);
   
   mrows = mxGetM(prhs[1]);
   ncols = mxGetN(prhs[1]);
   
   initvalues = mxCreateDoubleMatrix(mrows,ncols,mxREAL);
   
   step = mxGetScalar(prhs[3]);
   
   result = Runge_Kutta(func, initvalues, epoch, step);
...
//    
//     //Embedded_Verner_8_9
//     //( 0double (*f)(double, double), 1double y[],2double x,
//     //   3double h, 4double xmax, 5double *h_next, 6double tolerance )
//     
//    double (*func)(double, double);
//    double input[];
//    double initvalue[];
