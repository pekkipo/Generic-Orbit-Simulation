function [epoch, y0state, output_state, last_point_in_E] = srp_rkv89emb_maneuvers(f, t_range, y)
...
%                 end
              ...
              %if ~intermediate_maneuver   
                ...
                if ~isequal(sign(output_state(2,end-1)), sign(L2state(2,1)))  %&& t >= t_range+7.884e+6;%&& (L2state(1,1) < 0)
                   ...
                   [desired_t_for_maneuver, state_at_desired_t, state_at_desired_t_E ] = srp_find_T_foryzero( [epoch(end-1) epoch(end)], E_output_state(:,end-1), ytol);                  
